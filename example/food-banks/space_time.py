#space_time.py
#program requires 3 csv files: node, road_link, agent
...
def g_draw_space_time_diagram():
    ...
    color_value = { #see agent.csv column agent_type
        'high-speed': 'g', 
        'normal-speed': 'b', 
        'low-speed':'r',
        'lower-speed':'y',
    ...
    for i in range(0, len(g_agent_list)):
        ...
        plt.plot(g_agent_list[i].xlist, g_agent_list[i].ylist, color = color_value[agent_type], linewidth = 1.5, marker="o", markersize=4)
    ...
    plt.ylabel('Space (nodes)', family = 'Times new roman', fontsize= 12)
    plt.show()
